...
function Timer() {
  // Anniversary date
  const START_DATE = new Date('2014-07-09');
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const years = Math.abs(differenceInYears(now, START_DATE));
      const months = Math.abs(differenceInMonths(now, START_DATE)) % 12;
      const days = Math.abs(differenceInDays(now, START_DATE) % 30); // Approximation of days in a month
      ...
      <div className="text-center z-10">
        ...
        <div className="flex items-center justify-center gap-3 sm:gap-4 text-5xl sm:text-7xl font-bold mb-6">
          <div className="flex flex-col items-center">
            <span className="drop-shadow-lg">{timeLeft.years.toString().padStart(2, '0')}</span>
            <span className="text-sm sm:text-2xl mt-1 sm:mt-2">Years</span>
          </div>
          <span className="text-4xl sm:text-6xl">:</span>
          <div className="flex flex-col items-center">
            <span className="drop-shadow-lg">{timeLeft.months.toString().padStart(2, '0')}</span>
